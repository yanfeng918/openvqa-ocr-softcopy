...
def train_engine(__C, dataset, dataset_eval=None):
    ...
    for epoch in range(start_epoch, __C.MAX_EPOCH):
        ...
        for step, (
            ...
            ocr_glove_ix_iter = ocr_glove_ix_iter.cuda()
            ocr_fast_text = ocr_fast_text.cuda()
            ocr_phoc_frn = ocr_phoc_frn.cuda()
            ocr_bbox_iter = ocr_bbox_iter.cuda()
            keep_1 = keep_1.cuda()
            ...
            for accu_step in range(__C.GRAD_ACCU_STEPS):
                ...
                sub_ocr_glove_ix_iter = \
                    ocr_glove_ix_iter[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ocr_fast_text = \
                    ocr_fast_text[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ocr_phoc_frn = \
                    ocr_phoc_frn[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_ocr_bbox_iter = \
                    ocr_bbox_iter[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
                sub_keep_1 = \
                    keep_1[accu_step * __C.SUB_BATCH_SIZE:
                             (accu_step + 1) * __C.SUB_BATCH_SIZE]
